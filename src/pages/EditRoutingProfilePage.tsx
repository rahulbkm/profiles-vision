...
import { routingProfilesData, WorkClassificationRuleset } from '../data/routingProfiles';
...
function EditRoutingProfilePage() {
  const { profileId } = useParams<{ profileId: string }>();
  const navigate = useNavigate();
  
  const profile = routingProfilesData.find(p => p.id === profileId);
  
  const [name, setName] = useState(profile?.name || '');
  const [description, setDescription] = useState(profile?.description || '');
  const [workClassification, setWorkClassification] = useState<WorkClassificationRuleset[]>(
    profile?.configurations.workClassification || []
  );
  const [routeToQueueRules, setRouteToQueueRules] = useState<{ id: string; name: string; url: string } | null>(
    profile?.configurations.routeToQueueRules || null
  );
  const [fallbackQueue, setFallbackQueue] = useState(profile?.configurations.fallbackQueue || '');
  const [ruleHitPolicy, setRuleHitPolicy] = useState<'hit-first' | 'hit-all'>(
    profile?.configurations.ruleHitPolicy || 'hit-first'
  );
  const [workDistributionMode, setWorkDistributionMode] = useState<'push' | 'pick'>(
    profile?.configurations.workDistributionSettings.workDistributionMode || 'push'
  );
  const [capacityProfile, setCapacityProfile] = useState(
    profile?.configurations.workDistributionSettings.capacityProfile || ''
  );
  const [allowedPresences, setAllowedPresences] = useState<('available' | 'busy' | 'DND' | 'offline')[]>(
    profile?.configurations.workDistributionSettings.allowedPresences || ['available']
  );
  const [defaultSkillMatchingAlgorithm, setDefaultSkillMatchingAlgorithm] = useState<'closest-match' | 'exact-match' | 'none'>(
    profile?.configurations.workDistributionSettings.defaultSkillMatchingAlgorithm || 'none'
  );
  const [afterCallWorkMode, setAfterCallWorkMode] = useState<'always-block' | 'do-not-block' | 'custom-time'>(
    profile?.configurations.workDistributionSettings.afterCallWork.mode || 'always-block'
  );
  const [customTimeSeconds, setCustomTimeSeconds] = useState<number>(
    profile?.configurations.workDistributionSettings.afterCallWork.customTimeSeconds || 60
  );
  ...
  const handleSaveAndClose = () => {
    console.log('Saving routing profile:', {
      name,
      description,
      workClassification,
      routeToQueueRules,
    ...
    alert('Routing profile saved successfully!');
  ...
  return (
    <div className="edit-routing-profile-page">
      ...
      <div className="page-header">
        <h1 className="page-title">Edit Routing Profile</h1>
...
export default EditRoutingProfilePage;
